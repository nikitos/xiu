...
/// Extract audio/video elementary streams from TS and remux them into separate TS outputs.
pub fn to_separate_ts<R: ReadTsPacket>(ts_reader: R) -> Result<(bytes::BytesMut, bytes::BytesMut)> {
    let mut video_muxer = TsMuxer::new();
    let mut audio_muxer = TsMuxer::new();

    let video_pid = video_muxer.add_stream(epsi_stream_type::PSI_STREAM_H264, bytes::BytesMut::new());
    let audio_pid = audio_muxer.add_stream(epsi_stream_type::PSI_STREAM_AAC, bytes::BytesMut::new());
    ...
    while let Some(pes) = track!(reader.read_pes_packet().map_err(Error::from))? {
        ...
        let payload = bytes::BytesMut::from(&pes.data[..]);

        if pes.header.stream_id.is_video() {
            let mut flags = 0;
            if let Ok(nal_units) = ByteStreamFormatNalUnits::new(&pes.data) {
                for nal in nal_units {
                    if let Ok(unit) = NalUnit::read_from(nal) {
                        if unit.nal_unit_type == NalUnitType::CodedSliceOfAnIdrPicture {
            ...
            _ = video_muxer.write(video_pid.as_ref().unwrap().clone(), pts.as_u64() as i64, dts.as_u64() as i64, flags, payload);
        } else if pes.header.stream_id.is_audio() {
            _ = audio_muxer.write(audio_pid.as_ref().unwrap().clone(), pts.as_u64() as i64, dts.as_u64() as i64, 0, payload);
        }
    }
...
/// Make fragmented MP4 initial segment.
pub fn make_initialization_segment(
...
/// Make fragmented MP4 segment.
pub fn make_media_segment(avc_stream: AvcStream, aac_stream: AacStream) -> Result<MediaSegment> {
    ...
    segment.add_sidx(1, 1, 2);
    segment.add_sidx(2, aac_stream.adts_header.sampling_frequency.as_u32(), audio_duration as u64);

    Ok(segment)
}
/// AvcStream struct represents an AVC stream extracted from TS packets, containing the ADTS header, samples, and raw data.
#[derive(Debug)]
...
/// AacStream struct represents an AAC stream extracted from TS packets, containing the ADTS header, samples, and raw data.
#[derive(Debug)]
...
/// Reads TS packets from `reader`
pub fn read_avc_aac_stream<R: ReadTsPacket>(ts_reader: R) -> Result<(AvcStream, AacStream)> {
